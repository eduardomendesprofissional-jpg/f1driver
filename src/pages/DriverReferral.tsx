...
const DriverReferral = () => {
  ...
  const totalGanho = indicacoes
    ...
    .reduce((acc, i) => acc + Number(i.bonus_valor), 0);

  const pendentes = indicacoes.filter((i) => i.status === "pendente").length;
  const concluidas = indicacoes.filter((i) => i.status === "concluida").length;
        ...
        <Card className="border-primary/30 bg-gradient-to-br from-primary/10 to-primary/5">
          <CardContent className="p-6 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Convide motoristas e passageiros. Quando eles completarem a primeira corrida, vocês dois ganham!
            </p>
          </CardContent>
        </Card>
