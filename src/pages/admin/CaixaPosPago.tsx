...
const CaixaPosPago = () => {
  const [pagamentos] = useState<Record<string, unknown>[]>([]);
  const table = useTable({ data: pagamentos, searchKeys: ["motorista", "data"] });
            ...
            <Table>
              ...
              <TableBody>
                {table.paginatedData.length > 0 ? table.paginatedData.map((p, i) => (
                  <TableRow key={i}>
                    <TableCell className="text-sm">{String(p.data)}</TableCell>
                    <TableCell className="text-sm">{String(p.motorista)}</TableCell>
                    <TableCell className="text-sm">{String(p.valorRecebido)}</TableCell>
                    <TableCell className="text-sm">{String(p.situacao)}</TableCell>
                    <TableCell className="text-sm">{String(p.statusSaque)}</TableCell>
                  </TableRow>
                )) : (
                  <TableRow className="bg-muted/30">
